...
function init() {
    ...
    const fogColor = 0xFBA678; // Color de la niebla
    const fogNear = 0.1; // Cercanía donde empieza la niebla
    const fogFar = 15; // Distancia donde la niebla se vuelve densa
    scene.fog = new THREE.Fog(fogColor, fogNear, fogFar);

    // Ambient light
    const ambientLight = new THREE.AmbientLight(0xffccaa, 0.5); // Luz ambiental cálida
    scene.add(ambientLight);

    // Point light
    const pointLight = new THREE.PointLight(0x423CA9, 1, 100); // Luz puntual cálida
    pointLight.position.set(0, 3, 0); // Posiciona la luz
    scene.add(pointLight);

    // Directional light (sunlight)
    const dirLight = new THREE.DirectionalLight(0x201C64, 0.5); // Luz direccional cálida
    dirLight.position.set(5, 10, 5); // Posiciona la luz
    scene.add(dirLight);
...
new MTLLoader().setPath('model/SKYDOM/').load('CIELO1.mtl', function (materials) {
    ...
    controls.target.set(0, 0, 0); // Asegúrate de que los controles estén centrados en el origen
    controls.update();
...
// Esto evitará que el gato atraviese la pared
 console.log("¡El gato ha chocado contra la pared!");
    ...
    document.getElementById('contador_zanahoria').innerText = contador_zanahoria;
       ...
       // Actualizar el valor del input con el valor del contador
   
    const gato1 = document.getElementById('gato1');

    gato1.value = contador_zanahoria
    
    // Comprueba el valor del contador y muestra la ventana modal si es necesario
    if (contador_zanahoria === 5) {
        showModal("¡Ganaste!");
